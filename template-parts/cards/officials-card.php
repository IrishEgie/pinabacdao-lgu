...
if (!function_exists('officialCard')) {
    ...
    function officialCard($args = []) {
        ...
        $official_name = get_field('official_name', $args['post_id']);
        $position = $args['position'] ?: get_field('position', $args['post_id']);
        $department = get_field('department', $args['post_id']);
        $email = $args['email'] ?: get_field('email', $args['post_id']);
        $phone = $args['phone'] ?: get_field('phone', $args['post_id']);
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm group hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            ...
            <div class="p-6 pt-0 space-y-3">
                <?php if ($email) : ?>
                    <div class="flex items-center space-x-3 text-sm text-gray-600">
                        <?php echo display_service_icon('mail', 'w-4 h-4 text-blue-600'); ?>
                ...
                <?php if ($phone) : ?>
                    <div class="flex items-center space-x-3 text-sm text-gray-600">
                        <?php echo display_service_icon('phone', 'w-4 h-4 text-blue-600'); ?>
