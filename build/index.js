...
class NewsGallery {
  ...
  createLightboxDOM() {
    ...
    }).map((_, i) => `<button class="lb-dot w-2 h-2 rounded-full transition-all duration-300 ${i === 0 ? 'bg-white scale-125' : 'bg-gray-600 hover:bg-gray-400'}" data-index="${i}" aria-label="Go to image ${i + 1}"></button>`).join('')}
            </div>` : ''}
        `;
    ...
    if (this.totalSlides > 1) {
      div.querySelector('#lb-prev').addEventListener('click', e => {
        e.stopPropagation();
        this.prev();
      });
      div.querySelector('#lb-next').addEventListener('click', e => {
        e.stopPropagation();
        this.next();
      });
